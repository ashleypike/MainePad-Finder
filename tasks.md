## TASKS  

### Fully Functional Web Application  
*Deadline: 12/6/25*  
- Ashley: Home, Signup, and Login pages  
- Sophia: Listing, Property, and Messages pages  
- Jeffrey: Profile, and Matching pages  
- Yunlong: Manage Properties, Add Property, and Settings pages

### Advanced Datababase Feature Implementation  
*Deadline: 12/6/25*  
- Ashley: Database security & authentication  
- Sophia: Database indexing & query optimization  
- Jeffrey: Data warehousing or analytics features
- Yunlong: Transaction management / ACID compliance

### Performance Analysis & Optimization Report  
*Deadline: 12/8/25*  
- Ashley: Signup, and Login analysis  
- Sophia: Listing, and Property analysis 
- Jeffrey: Profile, and Matching analysis   
- Yunlong: Manage Properties, Add Property, and Settings analysis  

### User Manual & System Documentation  
*Deadline: 12/8/25*  
- Ashley: Installation/setup instructions  
- Sophia: Usage guide (Properties)  
- Jeffrey: Usage guide (Matching)  
- Yunlong: API Documentation  

### Recorded Video Demonstration  
*Deadline: 12/8/25*  
- Ashley: Application setup   
- Sophia: Database interactions  
- Jeffrey: Frontend functionality  
- Yunlong: Backend API calls  

### In-Class Presentation  
*Deadline: 12/8/25*  
- Ashley: Key features   
- Sophia: System architecture  
- Jeffrey: Lessons learned  
- Yunlong: Key features  
